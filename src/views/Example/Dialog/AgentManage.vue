...
const { tableRegister, tableState, tableMethods } = useTable({
  ...
  // 删除功能
  fetchDelApi: async () => {
    const res = await updateAppFieldApi({ fn: 'delete_agent', row: unref(ids) })
    return !!res
  }
})
// getList - 获取列表数据
...
const delData = async (row: TableData | null) => {
  ...
  ids.value = row
    ? [row.agent_user]
    : elTableExpose?.getSelectionRows().map((v: TableData) => v.agent_user) || []
...
const save = async () => {
  ...
  if (formData) {
    ...
    if (dialogTitle.value == t('exampleDemo.edit')) {
      ...
      res = await updateAppFieldApi({ fn: 'update_agent_field', row: formData })
    } else if (dialogTitle.value == t('exampleDemo.copy') || t('exampleDemo.add')) {
      delete formData.index
      delete formData.id
      res = await updateAppFieldApi({ fn: 'add_agent', row: formData })
    }
    if (res) {
      if (res.code != 0) {
